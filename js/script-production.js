...
function generateEmailAddress() {
    const subdomain = elements.subdomainInput ? elements.subdomainInput.value.trim() : '';
...
function newAddress() {
    ...
    if (email) {
        ...
        showNotification('Alamat email baru telah dibuat!', 'success');
        ...
        <div class="email-modal-content">
            ...
            <div class="email-modal-body">
                <div class="email-field">
                    ...
                    <div class="email-field-content">${getHeader('From')}</div>
                </div>
                <div class="email-field">
                    <strong><i class="fa-solid fa-envelope-open"></i> Subjek:</strong>
                    <div class="email-field-content">${getHeader('Subject')}</div>
                </div>
                <div class="email-field">
                    <strong><i class="fa-solid fa-calendar"></i> Tanggal:</strong>
                    <div class="email-field-content">${formatDate(getHeader('Date'))}</div>
                </div>
                <div class="email-field">
...
function initializeUI() {
    // Tambahkan input subdomain
    const emailControls = document.querySelector('.email-controls');
    if (emailControls) {
        const subdomainGroup = document.createElement('div');
        subdomainGroup.className = 'email-input-group';
        subdomainGroup.innerHTML = `
            <i class="fa-solid fa-globe input-icon"></i>
            <input id="subdomain" placeholder="subdomain (opsional)" />
            <small style="color: #6B7280; font-size: 12px;">Kosongkan untuk menggunakan netapp.my.id</small>
        `;
        
        emailControls.insertBefore(subdomainGroup, emailControls.querySelector('.button-group'));
        elements.subdomainInput = document.getElementById('subdomain');
        
        // Load saved subdomain
        const savedSubdomain = localStorage.getItem(CONFIG.SUBDOMAIN_STORAGE_KEY);
        if (savedSubdomain && elements.subdomainInput) {
